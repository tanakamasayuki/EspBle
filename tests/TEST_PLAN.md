...
| Address privacy (own address type) | | ✅ | ✅ `address_privacy` (random static advertising) | RPA rotation is outside automation (900 s period); bonded-peer resolution checked manually |
...
| HID keyboard event / layout | ✅ `unit/keymap` (Unicode 4-plane, AltGr, CapsLock) | ✅ | ✅ EN-US / JA-JP / en-GB / de-DE / fr-FR, modifiers | remaining layouts on hardware |
...
16. ✅ host unit tests (`tests/unit/`): keymap conversion (Unicode 4-plane, AltGr, per-character CapsLock pairs), HID Report Map parsing, codecs, the HCI router, and the HCI command scheduler.
...
22. ✅ `hid_keyboard_nkro`: 29-byte bitmap reports, eight simultaneous keys, high usages, individual release, and LEDs. `test_nkro_requires_mtu_32` checks the NKRO MTU floor: a 29-byte report needs `preferredMtu >= 32` (29 plus the 3-byte ATT header), so `begin()` rejects 23 and 31 with `InvalidArgument` and accepts 32. Failing silently would only look like a keyboard that sends nothing, so the error is explicit — and the MTU is not raised silently either, which would hide the configuration the application chose. The keyboard configuration must survive the rejection.
...
50. ✅ `hid_custom`: a vendor Report Descriptor composed through `ble.hidCustom()` — a 2-byte input, a 1-byte output and a 2-byte feature report under Report ID 1, three reports sharing UUID 0x2A4D. After discovery the generic GATT client resolves each report to its own handle, checks the Report Map (0x2A4B) length, subscribes to the input report by handle and decodes its 2 bytes, and writes the output report by handle so the device receives it in `onOutputReport()`. Each report's role is decided the way HID itself declares it: the Report Reference (0x2908) type byte (1=Input / 2=Output / 3=Feature), which can only be named through the `readDescriptor()` handle form because UUIDs cannot tell the three characteristics apart; the result's `descriptorHandle` and owning `handle` are checked against the discovery mapping. The declared type must also match the characteristic flags — input is notifiable, only output carries Write Without Response, and feature is acknowledged-write only, because a setting must not be fire-and-forget. `addFeatureReport()` gets an independent handle whose 2-byte write arrives verbatim at `onFeatureReport()`. The handle error contract is split: a zero handle is refused with `InvalidArgument` before acceptance, and an unknown handle fails after acceptance with `NotFound`.
...
53. ✅ `address_privacy`: the peripheral advertises a marker service with `EspBleConfig::ownAddressType = RandomStatic`, and the central scanner must observe it with addressType=Random (1) and the top two bits of the leading octet set to 0b11 (static random) — that is, not the public address.
...
56. ✅ `fitness_machine`: the standard Fitness Machine Service (0x1826). The server exposes Fitness Machine Feature (0x2ACC) for read and notifies Indoor Bike Data (0x2AD2) with flags 0x0044; the client reads the feature (features=6), subscribes, and decodes speed=3000, cadence=90 rpm and power=250 W in flag order. It then subscribes to the Control Point (0x2AD9) and Status (0x2ADA), sends Set Target Power (0x05, 250) and requires the response indication [0x80, 05, 01], and the server's 'g' triggers a "Target Power Changed" (0x08, 250) status notification (a single indication is verified, since one indication may be in flight per connection). Unsubscribing from Indoor Bike Data and disconnecting complete the suite.
...
58. ✅ `accept_list`: the peripheral fills its accept list with only an unreachable address and advertises under the `ConnectionFromAcceptList` policy, so the central's attempt must not connect and must return `onConnectionFailed` at the requested 4-second timeout — the backend takes about 30 seconds to give up on its own, so this checks the path where EspBle abandons the attempt and reports it. Restoring the `Any` policy lets the same central connect. The same list drives scan-side filtering through `EspBleScanConfig::acceptListOnly`: an empty list reports nothing, and adding the target address reports it. `acceptListEntry()` reads entries back, and `removeFromAcceptList()` makes the filter miss again — the removal direction, not just the addition.
59. ✅ `local_identity`: what the peripheral reports through `localAddress()` / `localAddressType()` must match the address/addressType the central observes in scan. `setTxPower(-12)` and `setTxPower(9)` must be followed both by what the radio applied (`txPower()`) and by the Tx Power Level that goes on the air in advertising. Finally `disconnect(id, 0x16)` must arrive at the peer's `disconnectReason` as 0x16 — the backend's 0x200 offset is normalized.
...
64. ✅ `persistent_subscription_overflow`: overflowing the 16-record persistent-subscription registry is counted instead of silently dropped — a lost record simply fails to restore on reconnect and nothing else reveals it, so the counter is the only trace. One address cannot fill the registry: the central's 16-entry active subscription table fills first, and a `subscribe()` refused there never reaches the CCCD write, so no record is created either; reconnecting to the same address just refills the active table through automatic restore. The peripheral therefore changes identity between batches, switching `ownAddressType` from Public to `RandomStatic` across `end()`/`begin()`. The procedure — subscribe 12 (records 1–12, dropped=0) → disconnect (the active table is released, the records remain) → re-init the peer → reconnect and subscribe 5 (records 13–16 land, the seventeenth overflows) — must leave `droppedPersistentSubscriptionCount()` at 1. The seventeenth `subscribe()` itself succeeds (only the record is lost), which is exactly why the counter has to exist. Subscriptions are issued strictly one at a time: the GATT queue holds one in flight plus eight, so batching twelve would get numbers nine onward refused with `ResourceExhausted` before the air, turning this into a queue test.
65. ✅ `gatt_queue_purge`: what happens to outstanding GATT operations when the connection ends — both failure modes are silent until symptoms appear. (a) A `disconnect()` during an in-flight GATT operation is not rejected but deferred: returning false would read as "still connected" to the application that just asked to disconnect. (b) Queued, unstarted operations are dropped and each receives an explicit failure completion: swallowing them would leave the application waiting for callbacks that never come. One procedure shows both: queue four reads (one on the air, three queued), then call `disconnect()`. The three are dropped at request time with `InvalidState` and "connection closed before the queued GATT operation started", while the one on the air completes normally — its success arriving before the disconnect is the proof of deferral. `droppedEventCount()` must stay 0 (nothing was missed to event-queue overflow), and a later reconnect and discovery must work (no ATT slot is left held).
