...
def test_gatt_read_write(dut, peers):
    ...
    peripheral.expect(
        re.compile(rb"SERVER_WRITE id=\d+ value=central-write stored=1 context=loop"),
        timeout=20,
    )
    peripheral.expect(
        re.compile(rb"SERVER_WRITE id=\d+ value=central-no-response stored=1 context=loop"),
        timeout=20,
    )
