...
def test_connection_parameters(dut, peers):
    ...
    connected = dut.expect(re.compile(
        rb"CONNECTED id=(\d+) interval=(\d+) latency=(\d+) timeout=(\d+) context=(\w+)\r?\n"), timeout=20)
    assert int(connected.group(2)) > 0, "initial connection interval should be populated"
    assert connected.group(5) == b"loop"
    device.expect(re.compile(rb"PEER_CONNECTED id=(\d+) interval=(\d+)\r?\n"), timeout=20)
    ...
    updated = dut.expect(re.compile(
        rb"PARAMS_UPDATED interval=(\d+) latency=(\d+) timeout=(\d+) context=(\w+)\r?\n"), timeout=20)
    ...
    peer_updated = device.expect(re.compile(
        rb"PEER_PARAMS_UPDATED interval=(\d+) latency=(\d+) timeout=(\d+) context=(\w+)\r?\n"), timeout=20)
