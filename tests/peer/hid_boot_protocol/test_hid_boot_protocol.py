...
def test_hid_boot_protocol(dut, peers):
    ...
    read = dut.expect(re.compile(rb"PROTOCOL_MODE_READ success=(\d+) mode=(\d+) context=(\w+)\r?\n"), timeout=20)
    ...
    mode = device.expect(re.compile(rb"PROTOCOL_MODE mode=(\d+) id=(\d+) context=(\w+)\r?\n"), timeout=20)
    ...
    boot = dut.expect(re.compile(
        rb"BOOT_INPUT length=(\d+) modifiers=(\d+) key=(\d+) context=(\w+)\r?\n"), timeout=20)
    ...
    output = device.expect(re.compile(rb"OUTPUT_REPORT leds=(\d+) caps=(\d+) context=(\w+)\r?\n"), timeout=20)
