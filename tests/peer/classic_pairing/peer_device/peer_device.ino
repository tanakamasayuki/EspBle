...
bool startStack()
{
  ...
  config.deviceName = "EspBle Pairing Peer";
  ...
  {
    Serial.printf("PAIR_PEER_BEGIN_FAILED error=%s detail=%s\n",
      bluetooth.lastErrorName(), bluetooth.lastErrorDetail().c_str());
    return false;
  }
  if (!bluetooth.spp().startServer())
  {
    Serial.printf("PAIR_PEER_SERVER_FAILED error=%s\n", bluetooth.lastErrorName());
    return false;
  }
  return true;
}
...
void setup()
{
  ...
  bluetooth.onNumericComparisonRequested(
    [](const EspBleClassicNumericComparison &event) {
      ...
      Serial.printf("PAIR_PEER_COMPARE peer=%s value=%06u auto=%u\n",
        event.peerAddress.c_str(), static_cast<unsigned>(event.value),
        autoAccept ? 1 : 0);
      if (autoAccept)
        Serial.printf("PAIR_PEER_CONFIRM accepted=%u\n",
  ...
  bluetooth.onSecurityChanged([](const EspBleClassicSecurityChanged &event) {
    Serial.printf("PAIR_PEER_SECURITY peer=%s success=%u status=%d\n",
  ...
  Serial.printf("PAIR_PEER_READY address=%02x:%02x:%02x:%02x:%02x:%02x\n",
...
void loop()
{
  ...
  if (Serial.available())
  {
    ...
    if (command == 'c')
    {
      Serial.printf("PAIR_PEER_CONNECT requested=%u\n",
        bluetooth.spp().connect(line.substring(1).c_str()) ? 1 : 0);
    }
    else if (command == 'b') reportBonds("PAIR_PEER_BONDS");
    else if (command == 'n')
    {
      autoAccept = false;
      Serial.println("PAIR_PEER_AUTO off");
    }
    else if (command == 'y')
    {
      autoAccept = true;
      Serial.println("PAIR_PEER_AUTO on");
    }
    else if (command == 'a')
    {
      Serial.printf("PAIR_PEER_ANSWER accepted=%u error=%s\n",
    ...
    {
      Serial.printf("PAIR_PEER_REJECT rejected=%u error=%s\n",
        bluetooth.confirmNumericComparison(
    ...
    else if (command == 'w')
    {
      ...
      Serial.printf("PAIR_PEER_STALE accepted=%u error=%s\n",
        bluetooth.confirmNumericComparison("00:00:00:00:00:00", true) ? 1 : 0,
        bluetooth.lastErrorName());
    }
    else if (command == 'x')
    {
      Serial.printf("PAIR_PEER_DELETE_ALL removed=%u count=%u\n",
        bluetooth.deleteAllBonds() ? 1 : 0,
        static_cast<unsigned>(bluetooth.bondCount()));
    }
    else if (command == 'd')
    {
      Serial.printf("PAIR_PEER_DISCONNECT requested=%u\n",
        sessionId != 0 && bluetooth.spp().disconnect(sessionId) ? 1 : 0);
    }
    else if (command == '?')
    {
      Serial.printf("PAIR_PEER_STATE sessions=%u bonds=%u heap=%u\n",
