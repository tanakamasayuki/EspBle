...
def test_user_data_service(dut, peers):
    ...
    age = dut.expect(re.compile(rb"AGE_READ valid=(\d+) value=(\d+) context=(\w+)\r?\n"), timeout=20)
    ...
    name_write = device.expect(
        re.compile(rb"SERVER_WRITE char=name length=(\d+) first=(\w) context=(\w+)\r?\n"), timeout=20)
    ...
    notify1 = dut.expect(re.compile(rb"DBCI_NOTIFY valid=(\d+) value=(\d+) context=(\w+)\r?\n"), timeout=20)
    ...
    age_write = device.expect(
        re.compile(rb"SERVER_WRITE char=age value=(\d+) context=(\w+)\r?\n"), timeout=20)
    ...
    notify2 = dut.expect(re.compile(rb"DBCI_NOTIFY valid=(\d+) value=(\d+) context=(\w+)\r?\n"), timeout=20)
    ...
    dut.write("a")
    dut.expect_exact("AGE_READ_REQUESTED", timeout=10)
    age2 = dut.expect(re.compile(rb"AGE_READ valid=(\d+) value=(\d+) context=(\w+)\r?\n"), timeout=20)
