...
void setup()
{
  ...
  if (!(passServiceService = server.addService(PASS_SERVICE_UUID)).valid() ||
      !(alertStatusCharacteristic = server.addCharacteristic(passServiceService, ALERT_STATUS_UUID, alertStatusConfig)).valid() ||
      !(ringerSettingCharacteristic = server.addCharacteristic(passServiceService, RINGER_SETTING_UUID, ringerSettingConfig)).valid() ||
      !(ringerControlPointCharacteristic = server.addCharacteristic(passServiceService, RINGER_CONTROL_POINT_UUID, controlConfig)).valid() ||
