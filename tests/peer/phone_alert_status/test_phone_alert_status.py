...
def test_phone_alert_status_service(dut, peers):
    ...
    status = dut.expect(re.compile(rb"ALERT_STATUS_READ valid=(\d+) status=([0-9a-f]{2}) context=(\w+)\r?\n"), timeout=20)
    ...
    initial = dut.expect(re.compile(rb"RINGER_SETTING valid=(\d+) value=(\d+) context=(\w+)\r?\n"), timeout=20)
    ...
    silent_cmd = device.expect(re.compile(rb"CONTROL_WRITE command=(\d+) context=(\w+)\r?\n"), timeout=20)
    assert int(silent_cmd.group(1)) == 1, "server should receive Set Silent Mode (1)"
    assert silent_cmd.group(2) == b"loop"
    silent = dut.expect(re.compile(rb"RINGER_NOTIFY valid=(\d+) value=(\d+) context=(\w+)\r?\n"), timeout=20)
    ...
    normal = dut.expect(re.compile(rb"RINGER_NOTIFY valid=(\d+) value=(\d+) context=(\w+)\r?\n"), timeout=20)
