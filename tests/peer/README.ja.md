...
- `hid_custom`: `ble.hidCustom()` で任意のvendor定義Report Descriptor（Report ID 1に2byte入力＋1byte出力をHID Serviceへ合成）から構成したCustom HIDデバイスを検証する。generic GATT clientがReport Map（0x2A4B）を読み34byte長とvendor usage pageの署名を確認し、入力Report（0x2A4D）を購読して2byteのカスタムReport（ダイヤル差分＋ボタン）をデコードする。任意Report Descriptor＋複数Reportの登録・駆動を確認する。
