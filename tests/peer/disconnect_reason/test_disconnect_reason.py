...
def test_disconnect_reason(dut, peers):
    ...
    device.expect(re.compile(rb"PEER_CONNECTED id=(\d+)\r?\n"), timeout=20)
    ...
    peer_disc = device.expect(
        re.compile(rb"PEER_DISCONNECTED id=(\d+) reason=(-?\d+) context=(\w+)\r?\n"), timeout=20)
    ...
    dut_disc = dut.expect(
        re.compile(rb"DISCONNECTED id=(\d+) reason=(-?\d+) context=(\w+)\r?\n"), timeout=20)
