...
READ_PATTERN = re.compile(rb"HOST_READ success=(\d+) len=(\d+)\r?\n")
QUERY_PATTERN = re.compile(rb"HOST_QUERY notifications=(\d+) connections=(\d+)\r?\n")
...
def test_hid_data_requires_encryption(dut, peers):
    ...
    match = dut.expect(re.compile(rb"HOST_DISCOVERED success=(\d+)\r?\n"), timeout=20)
    ...
    dut.expect(re.compile(rb"HOST_SUBSCRIBED success=(\d+)\r?\n"), timeout=20)
    time.sleep(0.5)
    device.write("k")
