...
void setup()
{
  ...
  if (!(linkLossServiceService = server.addService(LINK_LOSS_SERVICE_UUID)).valid() ||
      !(alertLevelCharacteristic = server.addCharacteristic(linkLossServiceService, ALERT_LEVEL_UUID, alertLevelConfig)).valid() ||
      !(txPowerServiceService = server.addService(TX_POWER_SERVICE_UUID)).valid() ||
      !(txPowerLevelCharacteristic = server.addCharacteristic(txPowerServiceService, TX_POWER_LEVEL_UUID, txPowerConfig)).valid() ||
  ...
  server.onWritten([](const EspBleGattWrite &write) {
    ...
    ble.gattServer().setValue(alertLevelCharacteristic, &level, sizeof(level));
