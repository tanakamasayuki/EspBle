...
- examples_compile: 公開APIと対象SoCのbuild回帰を検出する。現状はCIがなく、esp32s3 profileでの全example手動コンパイルを根拠とする（カバレッジ表のbuild列✅はこの手動確認を指す）。CI化はP0のTODO。
...
| 接続lifecycle / event queue / leak | | ✅ | ✅ `lifecycle_stress`（flood中切断、再接続heap、GATT中`end()`、connect中`end()`、scanner flush） | |
...
| reconnect / peer loss | state | ✅ | ✅ Bond再接続 / `lifecycle_stress`（peer lossは予定） | |
...
12. ✅ `lifecycle_stress`: event flood中の切断保持、再接続heap、GATT/connect実行中の`end()`、scanner flush。
