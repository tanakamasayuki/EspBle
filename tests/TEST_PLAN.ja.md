...
51. ✅ `hid_custom`: 任意Report DescriptorのCustom HID検証。`ble.hidCustom()`でvendor定義descriptor（Report ID 1に2byte入力＋1byte出力）をHID Serviceへ合成し、generic GATT clientがReport Map（0x2A4B）を読み34byte長とvendor usage page署名を確認、入力Report（0x2A4D）を購読して2byteのカスタムReport（ダイヤル差分+5・ボタン0x01）をデコードすることを確認。
