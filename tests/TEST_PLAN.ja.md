...
都合で3.3.8以上）。`--core-version`はこの範囲の下限側を実機で再確認するときに使います。
...
**書くときに止めてください。後からsuite全体を走査して足すのはやめてください。** それはここで
試して撤回しました。fieldが行末かどうかを機械的に判定するには2つが同時に成り立つ必要があり、
どちらも間違えやすいです。1つは、そのpatternが`expect`に渡されていて、先行する`expect`が
取得した文字列への`re.search`ではないこと。取得済みの文字列はそのexpectが止まった位置で
終わっているので、取得していない改行を要求すると永久に一致しません。もう1つは、そのpatternが
一致し**うるすべての行**でfieldが末尾であること。思い浮かべた1行だけでは足りません。
`DISCONNECTED id=(\d+)`は`DISCONNECTED id=1 context=loop`の内側にも一致し、sketchの書式が
`\n`で終わっていることは何の保証にもなりません。判定器を2つ書き、互いの盲点を潰しては別の
盲点を作り、2つ目はclean full runで30件を壊しました。patternを書く人はどの行のことか
知っていますが、後から走査するscriptは知りません。
