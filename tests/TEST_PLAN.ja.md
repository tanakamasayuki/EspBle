...
| Advertising / Scan parser | 予定 | 予定 | `advertise_scan`予定 | generic scanner |
| connect / disconnect / timeout | 状態遷移 | 予定 | 予定 | |
| GATT read / write | codec | 予定 | 予定 | generic GATT app |
...
3. `connect_disconnect`: Connection identity、local role、切断理由。
4. `gatt_read_write`: 汎用GATT Server/Client。
