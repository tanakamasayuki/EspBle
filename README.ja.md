...
その後のHID双方向通信、BLE接続を維持したClassic host再attachまで実機検証済みです。共有command scheduler、host要求event maskのunion、再attach時HCI Resetの仮想完了、最後のhostがcontrollerを停止するlifecycleも実装済みですが、その他のcontroller-wide commandの統合、長時間・security負荷試験は未完了です。詳細は
