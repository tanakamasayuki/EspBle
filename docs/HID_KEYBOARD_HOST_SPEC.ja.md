...
3. `ble.hidKeyboardHost().discover(connectionId)`を呼ぶ。security有効構成では`onSecurityChanged`の成功後に呼ぶ（市販keyboardはHID属性に暗号化を要求するため、暗号化前のDiscoveryは失敗しうる）。security無効構成では`onConnected`後に呼んでよい。
4. `onDiscovered()`で利用可能になったことを受け取る。
5. `onKeyboardState()`でHID usage snapshotを受け取る。

Discoveryの自動化は行わず、明示`discover()`が仕様です。再接続時も新しいConnection IDで同じタイミングで呼び直します。
...
- 同一report内ではpressをusage昇順、その後releaseをusage昇順で配送（一般的なOS Hostのchord処理とは順序が異なる仕様。順序に依存する用途は`onKeyboardState()`のsnapshotを使う）
...
writeはWrite Without Responseを優先し、ATT応答を待たずに`bool`を返すfire-and-forget helperです（WWR非対応のOutput Reportのみwith responseへfallbackします）。戻り値`true`は送信の受理を表し、Deviceへの配達確認ではありません。HID Discoveryなど他のGATT操作の実行中は排他により失敗を返すため、`onDiscovered()`受信後に呼びます。
