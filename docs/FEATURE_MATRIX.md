...
automatically selects the custom-built Classic-only Bluedroid host. That precompiled host is ABI-bound to
ESP-IDF 5.5.5 / xtensa-esp32 GCC 14.2.0; the measured core range is Arduino-ESP32 3.2.0 through 3.3.11, with HFP audio (SCO) needing 3.3.8 or newer. Classic has shipped since 1.3.0 with no support or compatibility guarantee; what each ⚠️ row states is what has been verified on hardware, and interoperability with external devices is unverified. Dual-host support remains experimental.
...
| Classic HID (BT HID) | ⚠️ | Classic-only generic Device/Host with the same profile API shapes as BLE (keyboard / mouse / consumer / system / gamepad), host-side Report Descriptor parsing, and the control channel (Get_Report / Set_Report / protocol mode / idle rate / virtual cable unplug) verified on hardware. How many profiles can be composed is capped by the SDP record at 214 bytes of descriptor plus strings — exceeding it is refused by `begin()` — and the host side decodes keyboard and mouse only. Use HOGP on BLE |
