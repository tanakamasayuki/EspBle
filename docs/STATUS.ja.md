...
- 外部NimBLE-Arduinoは対象外です。無印ESP32はEspBleがNimBLE Hostを同梱して動かします（`src/nimble_esp32/`）。独自buildしたClassic-only Bluedroid hostではSPP、generic HID Device/Host、A2DP Sink / Source、AVRCP CT/TGに加えてHFP Client/Audio GatewayのSLC、発信・着信・応答・終了、raw SCO transportを公開API化しました。ESP32同士でA2DPとAVRCPの併用、公開HFP Client/AG間のmSBC双方向payload、bad-frame、packet statistics、process-wide role排他を実機確認済みです。AVRCP metadata/play-statusは外部Target、HFPは外部機器相互運用を残します。dual-hostはHIDを中心に技術検証済みですがrelease scopeとACL credit一元管理が未確定のため一般対応とはしません。EspBleはencode済みmedia/SCO payloadまでを扱い、PCM処理・codec・device I/OはPCMFlowBluetooth等へ分離します。配布はNimBLE source / Classic `.a`のmixed方式を意図的に維持します。詳細は[Classic計画](PLAN_ESP32_CLASSIC.ja.md#配布形式の方針)と[Classic Audio計画](PLAN_ESP32_CLASSIC_AUDIO.ja.md)にあります。
