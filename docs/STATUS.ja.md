...
- 外部NimBLE-Arduinoは対象外です。無印ESP32はEspBleがNimBLE Hostを同梱して動かします（`src/nimble_esp32/`）。独自buildしたClassic-only Bluedroid hostではSPP、generic HID Device/Host、A2DP Sink / Sourceのraw transportに加え、AVRCP CT/TGのpassthrough、metadata/play-status要求、absolute volumeを公開API化しました。ESP32同士でA2DP external-codec転送と同時にPlay操作、応答、音量設定・変更通知を実機確認済みです。metadata/play-status応答はESP-IDF公開TG APIに送信手段がないため外部Targetとの確認を残します。dual-hostはHIDを中心に技術検証済みですがrelease scopeとACL credit一元管理が未確定のため一般対応とはしません。EspBleはencode済みmedia/SCO payloadまでを扱い、PCM処理・codec・device I/OはPCMFlowBluetooth等へ分離します。配布はNimBLE source / Classic `.a`のmixed方式を意図的に維持します。詳細は[Classic計画](PLAN_ESP32_CLASSIC.ja.md#配布形式の方針)と[Classic Audio計画](PLAN_ESP32_CLASSIC_AUDIO.ja.md)にあります。
...
1. AVRCP metadata/play-statusを外部Targetと相互運用確認し、HFP Client/AGのtransport APIを順に
   実装・実機検証する。AVRCPのpassthroughとabsolute volumeはESP32同士で確認済み。
