...
1. AVRCP、HFP Client/AGのtransport APIを順に実装・実機検証する。A2DP Sourceはcopy送信と
   `WouldBlock` retryを100 packetの実機転送で確認済み。
