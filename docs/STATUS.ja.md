...
- 接続パラメータ更新、実行時passkey、Numeric Comparisonは未対応です。切断理由は`EspBleConnection::disconnectReason`で取得できます。
