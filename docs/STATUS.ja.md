...
- 同梱NimBLE backendには、GATT client discoveryを反復すると解放されないヒープリーク（discoverしたcharacteristic数に比例、約2.6 KB/discovery）があります。詳細は[upstream報告案](UPSTREAM_REQUEST_ARDUINO_ESP32_GATTC_DISCOVERY_LEAK.ja.md)を参照してください。**汎用GATT Clientはwrapperの`BLEClient`のremoteオブジェクトを作らなくなった**ため（discoveryもread/write/購読もNimBLEホストAPIへ直接発行する）、この経路の確保は起きません。HID Host / MIDI Hostは自前のdiscovery経路を持ち未変更です。Boot Protocolを既定offにしたのはこのリークを不要に増幅しないためです。
