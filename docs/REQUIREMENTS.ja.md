...
HID HostはReport MapとReport ReferenceをDiscoveryし、固定された特定deviceだけに依存しない構造とします。Keyboardは固定6KRO、MouseとGamepadはdescriptor-driven field解析を行います。初期対応するReport Mapの範囲はHID仕様書で固定します。

## 優先順位付き拡張

初期基盤の完成後、需要、実装量、Peerテスト可能性を確認して1機能ずつ追加します。

1. 任意DescriptorのCustom HID、NKRO、Boot Protocol切替
