...
- 複合HID Device（Keyboard / Mouse / Consumer Control / System Control / Gamepad / Vendor、BLE Peripheral / GATT Server）
- HID Host（BLE Central / GATT Client、上記Report種別を横断Discovery）
- HID Output / Feature Report（keyboard LED、Vendor双方向Report）
- HIDへ組み込むBattery Service ServerとHostからのBattery Level取得

HID HostはReport MapとReport ReferenceをDiscoveryし、固定された特定deviceだけに依存しない構造とします。Keyboardは6KRO / NKRO、MouseとGamepadはdescriptor-driven field解析を行います。対応するReport Mapの範囲はHID仕様書で固定します。

## 優先順位付き拡張

初期基盤の完成後、需要、実装量、Peerテスト可能性を確認して1機能ずつ追加します。

1. （実装済み: 任意DescriptorのCustom HID、Boot Protocol切替）
