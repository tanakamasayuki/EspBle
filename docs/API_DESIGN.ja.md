...
初期SecurityはNo Input / No OutputのJust WorksとLE Secure Connectionsを使用します。Securityは`begin()`前の設定で有効にし、接続時の自動PairingとBond保存を個別に指定できます。無効が既定値です。
...
Passkey入力・表示、Numeric Comparison、MITM、認証必須Characteristic、Pairing拒否UI、Privacyは未実装です。Pairing失敗理由のbackend code表現と、Bond操作を同期Resultのままにするかも今後確定します。
...
- 同期helperを初期版へ含める範囲
- Passkey/Numeric Comparison/MITMとPairing失敗理由のAPI
