...
現在の`discover()`は内部taskでReport Map read、Report Reference列挙、Input subscription、Battery readを行います。初期parserはmodifier + reserved + 6-key arrayの8-byte Input Reportだけを受理し、未知のNKROや複合reportを誤ってboot形式として扱いません。`setKeyboardLeds()`のwrite完了方式、Discoveryの自動開始、再接続時の再Discovery/cacheは実装経験後に調整します。詳細は[HID Keyboard Host仕様](HID_KEYBOARD_HOST_SPEC.ja.md)に記載します。
