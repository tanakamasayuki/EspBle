...
| 同一UUIDのService複数登録 | ✅ | **Peripheral側は登録可**: 属性テーブルを`ble_gatts_add_svcs()`で自前に組み、`addService()`が返すハンドルが対象を表す。**Central側も区別可**: discoveryを`ble_gattc_disc_all_svcs()`で行い、read / write / CCCD書き込みも属性ハンドルへ直接発行するため2つ目以降のServiceにも到達できる。Notificationは`BLE_GAP_EVENT_NOTIFY_RX`を値ハンドルで対応付ける。`duplicate_uuid` Peerでread・subscribe・notifyまで検証済み |
| 同一Service内の同一UUID Characteristic | ✅ | **Peripheral側も登録可**: 属性テーブルを`ble_gatts_add_svcs()`で自前に組み、対象はaccess callbackが渡す定義ポインタで識別する。**Central側も区別可**（discoveryが属性ハンドル単位で列挙し、操作もハンドル指定）。`duplicate_uuid` Peerで、1つのServiceに同一UUIDのCharacteristic 2つ＋同一UUIDのService 2つを公開し、3つすべてへのread・subscribe・notifyを検証済み |
| GATT Client（一覧/既知UUID Discovery・Read・Write） | ✅ | 接続ごとのdiscovery snapshot、Descriptor操作（UUID指定と**属性ハンドル指定**の両方。HIDのReport Reference 0x2908のように、UUIDが重複するcharacteristicに属するdescriptorはハンドル指定でしか特定できない）、Write Without Response、操作単位timeout、操作の自動キュー（実行中1件＋8件）。汎用Client操作は同梱wrapperのremoteオブジェクトを使わずNimBLEホストAPIへ直接発行する（UUID重複への対応とdiscoveryのヒープリーク解消のため） |
...
| HFP（ハンズフリー） | ⚠️ | ClientのSLC/call control/CVSD・mSBC raw SCO transportは実機確認済み。AG公開APIは未実装 |
