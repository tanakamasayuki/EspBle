...
| Central接続 / Peripheral接続受け入れ / 切断 | ✅ | Scan Result/address直接接続、Connection ID管理、複数同時接続（容量まで）、auto-reconnect（`setAutoReconnect`、既定off） |
...
| 機能 | 状況 | 備考 |
|---|---|---|
| Beacon（non-connectable broadcaster） | ✅ | `setConnectable(false)`＋`setScanResponseEnabled(false)`＋`setInterval()`。payloadは`setManufacturerData`等で構築。実機Peer検証済み。iBeacon/Eddystoneの固定レイアウトhelperは📝（payloadを自前で組む） |
