...
| その他の標準Sensor Service | — | 📝🔧 | 標準UUIDを自分でGATT Server登録すれば📝、profile helperは🔧 |
...
- USB由来機能のうち未対応のもの（Vendor/Custom HID、BLE MIDI、複数接続）は[DECISIONS.ja.md](DECISIONS.ja.md)の優先順位候補と整合しています。採用時にexampleとPeer/unitテストを同時に追加します。
