...
| Eddystone-URL | ✅ | backend非依存codec `EspBleEddystone.h`（`espBleEncodeEddystoneUrl`/`espBleDecodeEddystoneUrl`）。Service Data（0xFEAA、frame 0x10、scheme＋サフィックス圧縮＋TX power）。`setServiceData()`送信・`EspBleScanResult::serviceData`受信。unit test＋`eddystone` Peerで検証済み。UID/TLM/EID frameは未対応 |
