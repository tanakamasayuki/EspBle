...
| 同一Service内の同一UUID Characteristic | ⚠️ | **Peripheral側は汎用GATT Server APIでは登録不可**: 同梱wrapperの`BLEService::addCharacteristic()`が既存を再利用して新しい方をGATTに登録しない。**wrapperの制約でありNimBLEの制約ではない**（EspBleのHID Deviceは`ble_gatt_chr_def`を自前で組み立てて同一UUIDのReport 0x2A4Dを複数公開できている）。**Central側は区別可**（自前discoveryが属性ハンドル単位で列挙し、操作もハンドル指定。HID Reportの撃ち分けを`hid_custom` Peerで検証済み）。`addCharacteristic()`が明示的にInvalidArgumentで拒否する（黙って動かない状態を避ける）。報告案は[UPSTREAM_REQUEST_ARDUINO_ESP32_NIMBLE_WHITELIST.ja.md](UPSTREAM_REQUEST_ARDUINO_ESP32_NIMBLE_WHITELIST.ja.md)の補遺 |
