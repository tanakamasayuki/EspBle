...
| 機能 | 状況 | 備考 |
|---|---|---|
| Legacy Advertising | ✅ | name / Service UUID / Manufacturer Data、connectable/non-connectable、Advertising間隔制御 |
...
| Notify / Indicate（購読・解除・CCCD） | ✅ | `Gatt/NotifyServer`・`Gatt/IndicateServer`。persistent subscription（`EspBleConfig::persistentSubscriptions`、既定on）で再接続時に自動再購読 |
| MTU交換 / payload上限検証 | ✅ | グローバルGAPイベント（`BLE_GAP_EVENT_MTU`）で両役割とも追跡し`onMtuChanged`へ配送。Central側の接続確立後のMTU交換も反映 |
...
| Eddystone URL / UID / TLM | ✅ | backend非依存codec `EspBleEddystone.h`。URL（frame 0x10、scheme＋サフィックス圧縮＋TX power）、UID（namespace 10B＋instance 6B＋TX power）、TLM（battery mV・8.8温度・adv count・uptime）。Service Data 0xFEAA。`setServiceData()`送信・`EspBleScanResult::serviceData`受信。unit test＋`eddystone` Peer（3 frame）で検証済み。EID frameは未対応 |
| Advertising Service Data（AD 0x16） | ✅ | `EspBleAdvertising::setServiceData(uuid, data, length)`送信、`EspBleScanResult::serviceData`/`serviceDataUuid`/`hasServiceData()`受信 |
