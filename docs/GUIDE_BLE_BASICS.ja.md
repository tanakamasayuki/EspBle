...
送信電力を上げれば距離が伸び、下げれば電池が持ちます。この設定を**EspBleは公開していません**。backend自体は設定APIを持っているため、必要になれば追加できます。現状はbackendの既定値で動作します。

#### 自分のアドレスの取得

自分がどのアドレスで通信しているかを**取得できません**。backendは取得APIを持っているため、必要になれば追加できます。

相手のFilter Accept Listへ自分を登録してもらうには自分のアドレスが必要ですが、現状は相手側でスキャンして調べる（[Info/ScanDump](../examples/Info/ScanDump/)）ことになります。
...
自分から切断するとき、相手へ伝える理由コードを**指定できません**。固定の理由で切断されます。逆に、相手から切断されたときの理由コードは受け取れます。
