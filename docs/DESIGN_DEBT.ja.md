...
4. **`connect()` timeoutがfiltered peerで効かない**: `cancelExpiredConnectAttempt()` は要求timeout経過時に `ble_gap_conn_cancel()` を呼んで backend内部の30秒待ちを打ち切る設計だが、**相手のcontrollerがFilter Accept Listで接続要求を捨てている場合に打ち切れない**。`accept_list` Peerで、timeout 4000 ms を指定した `connect()` の失敗検出に実測 **約31秒**（＝backend既定の30秒）かかることを確認。到達不能アドレスへの接続では従来どおり動作しているように見えるため、cancelがこの状況で失敗している（戻り値を見ていない）可能性が高い。→ `ble_gap_conn_cancel()` の戻り値を確認し、失敗時の代替手段（再試行、client破棄）を検討する。**状況: 未着手**（`accept_list` テストはこの挙動に合わせ45秒待ちにしてある）
