...
- **setKeyboardLeds / sendVendorReport もqueue化（DECISIONS HID Host #6 の同期契約は破棄）**: 正しさ基準では「ATTに触れる全操作がqueueで直列化し、`gattOperating` 中でも失敗しない」不変条件を全HID Host操作へ徹底すべき。旧実装は同期WWRをloop task上でblocking実行し、busy時にfalseを返していた（concurrent client accessの安全性から同期のままでは根絶不可）。両者を汎用Write（handle指定）としてqueueへ載せ、戻り値は「受理」を表す（配送は `onCharacteristicWritten`）。`hid_keyboard_host` の `HOST_LEDS_TIMED` テストは「enqueue呼び出しが非blockingであること（update()での実送信は除外して合計を計測）」を検証する形へ更新済み。

**破壊的変更**: `discover()` の戻り値が「busy時false」→「常に受理true」。`setKeyboardLeds()`/vendor writeは同期実行→queue受理（戻り値=受理、配送は非同期）。
**テスト影響**: `lifecycle_stress`（`test_reconnect_cycles_do_not_leak_heap` が再接続時の自動再購読とdiscoverの共存を検証する回帰テストになる）、`hid_keyboard_host`（`HOST_LEDS_TIMED` をenqueue非blocking検証へ更新）、`hid_*` 一式。
**状況: 完了（要実機再確認）**
