...
**状況: 完了（実機検証済み。`notify_indicate` がbroadcast時の`connectionId=0`、queueされた連続3件、接続指定notify、indicateをassertしている）**
...
2. **切断時のqueue未purge＋GATT op中の `disconnect()` reject**: `removeConnection` が `gattQueue` を触らず、切断済み接続のqueue済みopが残って生存接続を遅延させる。`disconnect()` はGATT op中false。→ **完了**: `removeConnection` が `purgeQueuedGattOpsLocked(connectionId)` で当該接続のqueue済みopをdrop（generic opは失敗GattResultを配送して完了contractを維持、queued HidDiscoverはHID Host切断処理に委ねて静かにdrop、実行中opは無干渉）。`disconnect()` はGATT op中に**rejectせず deferred**（`ConnectionSlot::pendingDisconnect`＋`update()` の `drainPendingDisconnects()` でop完了後に実行）。

**実機検証時に、deferredが「op完了後」ではなく「キューが空になった後」になっていたことが判明し、あわせて是正した。** `update()` は `pumpGattQueue()` → `drainPendingDisconnects()` の順で走り、drainは「そのconnectionのopが実行中なら待つ」だけなので、pumpが毎回次のopを開始してしまい切断が後回しになる。キューに積み続けるアプリでは**無期限に飢餓**し、purge経路は `disconnect()` からは到達不能だった（切断時にはキューが空なので落とすものが無い）。是正: `disconnect()` の時点で当該connectionのqueue済みopを `purgeQueuedGattOpsLocked()` で落とす（失敗完了付き）。落とす相手を待つ作業を残す意味は無く、purge関数自身のコメント（「生存接続の前に詰まらせないため」）とも一致する。実行中のopは触らないので正常完了する。**状況: 完了（実機検証済み。`gatt_queue_purge`）**
3. **NKROのMTU下限未引き上げ**: `enableNkro()` はフラグとreport長29のみ設定し `preferredMtu`（既定23）を触らないため、29-byte notifyが送信時に無言失敗。→ **完了**: ライブラリの明示エラー方針に合わせ、`begin()` で「NKRO keyboard configured かつ `preferredMtu < 32`」を `InvalidArgument` で拒否（無言失敗より明示エラー。silentにMTUを上書きしない）。**状況: 完了（実機検証済み。`hid_keyboard_nkro` の `test_nkro_requires_mtu_32` が23・31を拒否、32を受理、拒否後もkeyboard構成が残ることを確認）**
