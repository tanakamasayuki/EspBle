...
- **setKeyboardLeds / sendVendorReport は対象外（意図的、DECISIONS HID Host #6）**: これらは同期のWrite Without Response fire-and-forget（`update()`非依存で連続10回が非blockingに成功する契約、`hid_keyboard_host` の `HOST_LEDS_TIMED` で担保）。queue化するとこの契約が壊れ（8枠queue溢れ・配送遅延）テストも破れるため、非同期Resultへは統一しない。GATT op進行中に false を返すのはこの同期契約の受容可能な帰結。

**破壊的変更**: `discover()` の戻り値が「busy時false」→「常に受理true」。
**テスト影響**: `lifecycle_stress`（`test_reconnect_cycles_do_not_leak_heap` が再接続時の自動再購読とdiscoverの共存を検証する回帰テストになる）、`hid_*` 一式。
**状況: 完了（要実機再確認）**
