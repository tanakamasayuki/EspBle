...
- **`ble.begin()` のBTコントローラ初期化中のipc0タスクスタック超過（マージナル）**: コントローラのIRQ割り当てが `ipc0` タスク（`CONFIG_ESP_IPC_TASK_STACK_SIZE=1024`）上で `btdm_intr_alloc → esp_intr_alloc → heap_caps_malloc`（`HEAP_POISONING_LIGHT`）を実行し、そこへISRがネスト（`_xt_context_save`）するとスタック canary watchpoint（`FREERTOS_WATCHPOINT_END_OF_STACK`）が発火してパニック→再起動ループ。ヒープは潤沢（~299KB空き）でスタック超過が原因。バックトレース全frameとsdkconfigで確定。ISRネストのタイミング／バイナリ配置に依存する**マージナルな不具合**で、`hid_boot_keyboard` peer で顕在化（同一基板の他device binaryでは踏まない。デバッグprintを1行足す＝配置がずれるだけで回避される）。ipcタスクスタックもheap poisoningも**precompiled sdkconfigで固定のためアプリからは変更不可**。upstream（arduino-esp32でのipcスタック引き上げ）事項。EspBleロジックやsketch構成とは無関係。
