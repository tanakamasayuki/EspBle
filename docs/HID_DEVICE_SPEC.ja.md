...
- profileごとにInput Report `0x2A4D`とReport Reference `{id, 1}`を作ります。keyboardだけはLED Output Report `{1, 2}`も持ちます。
...
Output Report callbackはstack taskではなく`ble.update()`から配送します。Battery Levelはprofile共通のDevice情報として、最初に構成したprofileのconfigを採用します。

## 検証

`tests/peer/hid_keyboard_device`はArduino-ESP32 BLE APIを直接使うCentralから、keyboard+mouse複合Report Map、複数Report Reference、個別CCCD、8-byte keyboard通知、4-byte mouse通知、LED Output、Battery、暗号化を実機検証します。`tests/peer/hid_keyboard_host`では全5 profileを同時構成し、EspBle Hostとの相互運用を検証します。
