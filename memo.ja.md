...
* ESP32-H2
* ESP32-S3
* ESP32-P4（規定GPIOへESP32-C6などを接続するHosted BLE構成）
* その他、Arduino Core for ESP32とNimBLEを利用可能なESP32シリーズ

Bluetooth LEの内蔵有無だけで対象外にせず、Arduino Core for ESP32がNimBLEを利用可能にするHosted BLE構成も対象候補とする。チップおよびnative/Hosted構成ごとに利用可能なBluetooth LE機能、同時接続数、PHY、Extended Advertisingなどが異なる場合は、機能の有無を明示的に判定できることを要件とする。
