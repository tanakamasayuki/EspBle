...
sketch内の `ALLOWED_CENTRAL` を、**接続を許可したいCentralのアドレス**に書き換えてから使ってください。書き換えないままだと誰も接続できません（それ自体、フィルタが効いていることの確認にはなります）。
...
- 拒否された相手には「拒否された」と伝わりません。接続要求が無視されるだけなので、相手側からはタイムアウトに見えます。
